    ...
    angular.module('globalApp').service('adobeTracking', ['$rootScope', '$window', '$q', 'registry', '$route', function ($rootScope, $window, $q, registry, $route) {
        ...
        return {
            ...
            track: function (event) {
                ...
                if (event.name == "jobView" && $rootScope.configLayout.redirectToJv30.lookUpConfig.value == "true")
                    return;
        ...
        function trackPpcClick(data) {
            if (typeof data === 'object') {
                if (data !== undefined) {
                    ...
                    imgCreator(track.clicks, mappedJob, function () {
...
(function (angular) {
    ...
    var ppcMetaTags = 'ppcMetaTags';
    var jobViewedAttr = 'data-m_impr_a_p';

    angular.module('globalApp').service('ppcTrackingService', ['$rootScope', '$q','$timeout','$window', function ($rootScope, $q, $timeout, $window) {   
        return {
            insertPPCMetaTags: function (query, keyword, location, page, affiliateId) {
                
                //var a_p = $('a[' + jobViewedAttr + ']');

                //if (a_p.length > 0) {
                //    angular.forEach(a_p, function (item, key) {
                //        $(item).removeAttr(jobViewedAttr);
                //        console.log(item);
                //    });
                //}
                ...
                // Add PPC Meta Tags to html header section
                var head = document.getElementsByTagName('head')[0];
                head.appendChild(createmyMetaElement("m_impr.s_search_query", query));
                head.appendChild(createmyMetaElement("m_impr.s_q", keyword));
                head.appendChild(createmyMetaElement("m_impr.s_where", location));
                head.appendChild(createmyMetaElement("m_impr.j_pg", page));
                head.appendChild(createmyMetaElement("m_impr.a_affiliate_id", affiliateId));

                var nowDate = new Date().getTime();

                //var scriptPath = 'http://logs.jobs.com/impression_tracking.js?nocache=' + nowDate;
                //debug version
                var scriptPath = 'http://10.60.53.131/impression_tracking.js?nocache=' + nowDate;

                // Add PPC Script to body
                var body = document.getElementsByTagName('body')[0];
                var script = document.createElement("script");
                script.setAttribute("type", "text/javascript");
                script.src = scriptPath;
                script.setAttribute("class", ppcMetaTags);
                ...
                //        t.trackingBaseUrl = "//10.60.53.131/";
                //        var r = e.createElement("script");
                //        r.type = "text/javascript";
                //        r.async = true;
                //        r.src = "//10.60.53.131/impression_main?v=HeeUa2qXiTht7vajd3XimTsE5rJ3ZQC-7SscFbdg8Qk1";
                //        var i = false;
                ...
                //})(document, window.mnstr_imprtrack = window.mnstr_imprtrack || {});
                
                $window.dispatchEvent(new Event('load'));

                console.log(window.mnstr_imprtrack);
    ...
    function removeJobViewed() {

    }

    function createmyMetaElement(name,value){
        var meta = document.createElement('meta');
        meta.setAttribute('class', ppcMetaTags);
...
(function () {
    ...
    function timeoutManager($rootScope, $timeout, $interval, authentication, $uibModal, registry) {
        ...
        function _manageLoggedOutModal() {
            registry.set('public', 'showLoggedOutModal', true, 'localStorage');
            service.LoggedOutModalInstance = $uibModal.open({
                animation: true,
                templateUrl: 'modalTimeoutLoggedOut.html',
                controller: 'ModalInstanceCtrl',
...
(function () {
    ...
    function authenticationStorage($window, $rootScope, registry) {
        var factory = {};

        function getKey(key) {
            return $rootScope.registry.sessionStore.authentication.ChannelID.toString() + ':' + key;
            //return $rootScope.productVariables.ChannelID.toString() + ':' + key;
        }

        factory.getAuthenticationType = function () {
            try {
                return $rootScope.registry.sessionStore.authentication.login_type_key;
            } catch (err) {
                return null;
            }
        ...
        factory.setAuthenticationType = function (type) {
            registry.set('authentication', 'login_type_key', type, 'sessionStorage');
        ...
        factory.setProlongDate = function (date) {
            ...
            registry.set('authentication', 'prolong_date_key', date.getTime(), 'sessionStorage');
            //$window.sessionStorage.setItem(getKey('prolong_date_key'), date.getTime());
        };

        factory.getProlongDate = function () {
            try {
                var milliseconds = $rootScope.registry.sessionStore.authentication.prolong_date_key;
            }
            catch (err) {
                return null;
            }
        ...
        factory.getImpersonation = function () {
            var result = $rootScope.registry.sessionStore.authentication.webadmin_impersonation_key;
        ...
        factory.setImpersonation = function (impersonation) {
            ...
            registry.set('authentication', 'webadmin_impersonation_key', value, 'sessionStorage');
        ...
        factory.getToken = function () {
            ...
            if (type != null) {
                if (type == LOGIN.Lexus) {
                    try {
                        return $rootScope.registry.sessionStore.authentication.login_token_key;
                    }
                    catch (err) {
                        return null
                    }
        ...
        factory.setAuthenticateUsingSaml = function (authWithSaml) {
            registry.set('authentication', 'authenticate_using_saml', authWithSaml, 'sessionStorage');
            //$window.sessionStorage.setItem(getKey('authenticate_using_saml'), authWithSaml);
        }

        factory.getAuthenticateUsingSaml = function () {
            try {
                return $rootScope.registry.sessionStore.authentication.authenticate_using_saml;
            }
            catch (err) {
                return null;
            }
        ...
        factory.setIdpAuthCookieName = function (cookieName) {
            registry.set('authentication', 'idp_auth_cookie_name', cookieName, 'sessionStorage');
            //$window.sessionStorage.setItem(getKey('idp_auth_cookie_name'), cookieName);
        }

        factory.getIdpAuthCookieName = function () {
            return $rootScope.registry.sessionStore.authentication.idp_auth_cookie_name;
            //return $window.sessionStorage.getItem(getKey('idp_auth_cookie_name'));
        }

        factory.clear = function () {
            $window.mgr.removeToken();
            registry.remove('authentication', 'login_token_key', 'sessionStorage');
            //$window.sessionStorage.removeItem(getKey('login_token_key'));

            registry.remove('authentication', 'webadmin_impersonation_key', 'sessionStorage');
            //$window.sessionStorage.removeItem(getKey('webadmin_impersonation_key'));

            registry.remove('authentication', 'login_type_key', 'sessionStorage');
            //$window.sessionStorage.removeItem(getKey('login_type_key'));

            registry.remove('authentication', 'prolong_date_key', 'sessionStorage');
            //$window.sessionStorage.removeItem(getKey('prolong_date_key'));

            registry.remove('authentication', 'user_key', 'sessionStorage');
            //$window.sessionStorage.removeItem(getKey('user_key'));

            registry.remove('authentication', 'redirect_key', 'sessionStorage');
            //$window.sessionStorage.removeItem(getKey('redirect_key'));

            registry.remove('authentication', 'authenticate_using_saml', 'sessionStorage');
            //$window.sessionStorage.removeItem(getKey('authenticate_using_saml'));

            registry.remove('authentication', 'idp_auth_cookie_name', 'sessionStorage');
            //$window.sessionStorage.removeItem(getKey('idp_auth_cookie_name'));
        };

        return factory;
    };
    authenticationStorage.$inject = ['$window', '$rootScope', 'registry'];
    ...
    function authenticationFactory($window, $resource, $rootScope, $timeout, $location, authenticationStorage, registry) {
        var factory = {};
    ...
    authenticationFactory.$inject = ['$window', '$resource', '$rootScope', '$timeout', '$location', 'authenticationStorage', 'registry'];
        ...
        return {
            ...
            responseError: function (response) {
                ...
                if (response.status != 400){
                    ...
                    $rootScope.topErrorList.push({ error: response.status, text: tempErrorString, type: 'alert-danger', message: (response.statusText) ? response.statusText: '', message2: (response.data.message)?response.data.message: ''});
                }
...
(function () {
    ...
    loginController.$inject = ['$scope', '$rootScope', '$httpParamSerializer', 'authentication', 'authenticationStorage', '$uibModal', '$window', '$location', '$http', '$cookies'];
    function loginController($scope, $rootScope, $httpParamSerializer, authentication, authenticationStorage, $uibModal, $window, $location, $http, $cookies) {
        ...
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                scope.moveToNextSection = function (clickedFrom, accordionMeta) {
                    scope.$broadcast("moveToNextSection", { clickedFrom: clickedFrom });
        ...
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                function ShowHide(currentLocation) {
                    if (apply && apply.id > 0 && ShowOnThisPage(currentLocation) && $rootScope.isAuthenticated) {
                        scope.show = true;
                        scope.job = apply;
                    }
                    else {
                ...
                link: function (scope, element, attrs) {
                    ...
                    var unwatch = scope.$watchGroup(['content', 'src'], function (type) {
                        ...
                        function applyStyles() {
                            ...
                            if (elem.find('iframe')[0].src.startsWith("/") && elem.find('iframe')[0].contentDocument) {
...
(function () {
    'use strict';
    angular.module('globalApp').directive('ocsUpload', ['$parse', '$rootScope', '$log', 'FileUploader', 'authenticationStorage', 'documentUpload', '_', 'templateUrlService', '$timeout', directiveFunction]);

    function directiveFunction($parse, $rootScope, $log, FileUploader, authenticationStorage, documentUpload, _, templateUrlService, $timeout) {
        return {
            restrict: "E",
            scope: {
                meta: "=",                    
...
(function () {
    ...
    angular.module('globalApp').run(['$rootScope', 'channelConfig', 'dataSourceService', 'resolveLayoutConfigs', '$q', 'cacheService', '_', 'menuCreator', 'resolveMetaMessages', 'resolveMetaConfigs', 'resolveDataSource', 'resolveTracking', 'timeoutManager', 'authenticationStorage', 'registry',
    function ($rootScope, channelConfig, dataSourceService, resolveLayoutConfigs, $q, cacheService, _, menuCreator, resolveMetaMessages, resolveMetaConfigs, resolveDataSource, resolveTracking, timeoutManager, authenticationStorage, registry) {
...
$templateCache.put("https://core.ui.lexus.monster.com/uiCore/templates/ocs-CheckBoxWithModal.html","<div class=\"mb15\">\r\n    <div>\r\n        <fieldset class=\'form-group\' ng-disabled=\"meta.disabled\">\r\n            <legend class=\"label-type\" ng-class=\"{\'sr-only\': meta.hideLabel}\">\r\n                <span ng-bind-html=\"meta.label.lookUpMsg.value\"></span>\r\n            </legend>\r\n            <div class=\"checkbox\" ng-repeat=\'n in meta.items | limitTo: meta.limit\' ng-class=\"{\'mb0\' : $last}\">\r\n                <label for=\"{{elementID}}-{{n.value}}\">\r\n                    <input type=\"checkbox\"\r\n                           id=\"{{elementID}}-{{n.value}}\"\r\n                           ng-model=\'$parent.trueModel[$index]\'\r\n                           name=\'{{elementID}}-{{n.value}}\'\r\n                           ng-true-value=\"{{n.value}}\"/>\r\n                    <span class=\"checkbox-custom\"\r\n                          style=\"height: calc(1em + 1px); width: calc(1em + 1px);\"\r\n                          ng-style=\"meta.items.length == 1 && {\'width\': \'calc(1em + 5px)\'}\"\r\n                          ng-class=\"{\'checkbox-toggle\': meta.items.length == 1}\"></span>\r\n                    <span class=\"checkbox-label\">{{n.text.lookUpMsg.value || n.text}}</span>\r\n                </label>\r\n            </div>\r\n        </fieldset>\r\n    </div>\r\n    <div ng-if=\"meta.items.length > meta.limit\">\r\n        <a href=\"\" ng-click=\"open()\">View more <span class=\"fa fa-caret-down\"></span></a>\r\n    </div>\r\n    \r\n\r\n    <script type=\"text/ng-template\" id=\"checkFilterModalContent.html\">\r\n        <div class=\"modal-header\">\r\n            <h4 class=\"modal-title\"><strong>All results for {{meta.label.lookUpMsg.value}}</strong></h4>\r\n        </div>\r\n        <div class=\"modal-body\">\r\n            <div class=\"row\">\r\n                <div class=\"col-sm-6\">\r\n                    <fieldset class=\'form-group\'>\r\n                        <legend class=\"label-type sr-only\">\r\n                            <span ng-bind-html=\"meta.label.lookUpMsg.value\"></span>\r\n                        </legend>\r\n                        <div class=\"checkbox\" ng-repeat=\'n in items.left\'>\r\n                            <label for=\"{{elementID}}-{{n.value}}\">\r\n                                <input type=\"checkbox\"\r\n                                       id=\"{{elementID}}-{{n.value}}\"\r\n                                       ng-model=\'model[$index]\'\r\n                                       name=\'{{elementID}}-{{n.value}}\'\r\n                                       ng-true-value=\"{{n.value}}\" />\r\n                                <span class=\"checkbox-custom\"\r\n                                      style=\"height: calc(1em + 1px); width: calc(1em + 1px);\"\r\n                                      ng-style=\"meta.items.length == 1 && {\'width\': \'calc(1em + 5px)\'}\"\r\n                                      ng-class=\"{\'checkbox-toggle\': meta.items.length == 1}\"></span>\r\n                                <span class=\"checkbox-label\">{{n.text.lookUpMsg.value || n.text}}</span>\r\n                            </label>\r\n                        </div>\r\n                    </fieldset>\r\n                </div>\r\n               <div class=\"col-sm-6\">\r\n                    <fieldset class=\'form-group\'>\r\n                        <legend class=\"label-type sr-only\">\r\n                            <span ng-bind-html=\"meta.label.lookUpMsg.value\"></span>\r\n                        </legend>\r\n                        <div class=\"checkbox\" ng-repeat=\'n in items.right\'>\r\n                            <label for=\"{{elementID}}-{{n.value}}\">\r\n                                <input type=\"checkbox\"\r\n                                       id=\"{{elementID}}-{{n.value}}\"\r\n                                       ng-model=\'model[$index + items.left.length]\'\r\n                                       name=\'{{elementID}}-{{n.value}}\'\r\n                                       ng-true-value=\"{{n.value}}\" />\r\n                                <span class=\"checkbox-custom\"\r\n                                      style=\"height: calc(1em + 1px); width: calc(1em + 1px);\"\r\n                                      ng-style=\"meta.items.length == 1 && {\'width\': \'calc(1em + 5px)\'}\"\r\n                                      ng-class=\"{\'checkbox-toggle\': meta.items.length == 1}\"></span>\r\n                                <span class=\"checkbox-label\">{{n.text.lookUpMsg.value || n.text}}</span>\r\n                            </label>\r\n                        </div>\r\n                    </fieldset>\r\n                </div>\r\n            </div>\r\n        </div>\r\n        <div class=\"modal-footer\">\r\n            <button class=\"btn btn-primary\" type=\"button\" ng-click=\"return(filter.popup)\">Save and return</button>\r\n        </div>\r\n    </script>\r\n</div>");
...
$templateCache.put("https://core.ui.lexus.monster.com/uiCore/templates/ocs-Error.html","<div class=\"alert {{x.type}}\" aria-role=\"alert\" ng-repeat=\"(key,x) in errorList track by $index\">\r\n    <button ng-click=\"$parent.popError(key)\"\r\n            tabindex=\"0\" \r\n            type=\"button\" \r\n            class=\"close\" \r\n            data-dismiss=\"alert\" \r\n            aria-label=\"Close\">\r\n        <span aria-hidden=\"true\">&times;</span>\r\n    </button>\r\n    <p>&nbsp;<span ng-bind-html=\"x.text | unsafe\"></span></p>\r\n</div>\r\n");
