...
(function () {
  ...
  .provider('lkGoogleSettings', function () {
    ...
    this.features = [];
    this.views    = ['DocsView().setIncludeFolders(true)'];
        ...
        function pickerResponse (data) {
          gapi.client.load('drive', 'v2', function () {
            ...
            if (data.action == google.picker.Action.PICKED && scope.onPicked) {
                ...
                request.execute(function(fileMeta) {
                    (scope.onPicked || angular.noop)({ fileInfo: { meta: fileMeta, token: gapi.auth.getToken().access_token } });
                });             
            }
            scope.$apply();
          });
